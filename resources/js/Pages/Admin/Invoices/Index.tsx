...
export default function Index({ invoices, activeCustomers = [] }: Props) {
    ...
    const [viewingProofPath, setViewingProofPath] = useState<string | null>(null);

    const rejectPayment = (id: number) => {
        if (confirm('Tolak bukti transfer dari pelanggan ini? File bukti transfer akan dihapus.')) {
            router.post(route('admin.invoices.reject-payment', id), {}, {
                preserveScroll: true
            });
        }
    };
        ...
        >
                                        ...
                                        <tr key={inv.id} className={`hover:bg-gray-50/30 transition-colors group ${selectedIds.includes(inv.id) ? 'bg-indigo-50/20 dark:bg-indigo-900/10' : ''}`}>
                                            ...
                                            <td className="px-6 py-5 text-center">
                                                {inv.status === 'paid' ? (
                                                    ...
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-cyan-50 text-cyan-600 rounded-full text-xs font-black uppercase tracking-tighter" title="Menunggu verifikasi admin">
                                                    ...
                                                    {inv.status === 'unpaid' && (
                                                        <>
                                                            {inv.payment_proof && (
                                                                <>
                                                                    <button 
                                                                        onClick={() => setViewingProofPath(inv.payment_proof || null)}
                                                                        className="flex items-center gap-1 px-3 py-2 bg-cyan-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-cyan-700 transition-all"
                                                                        title="Lihat Bukti Transfer"
                                                                    >
                                                                        <Eye className="w-3 h-3" /> Bukti
                                                                    </button>
                                                                    <button 
                                                                        onClick={() => rejectPayment(inv.id)}
                                                                        className="flex items-center gap-1 px-3 py-2 bg-rose-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-rose-700 transition-all"
                                                                        title="Tolak Bukti"
                                                                    >
                                                                        Tolak
                                                                    </button>
                                                            ...
                                                            >
                                                                <CreditCard className="w-3 h-3" /> {inv.payment_proof ? 'Setujui' : 'Pay'}
                                                            </button>
                                                        </>
                                                    )}
            ...
            {viewingProofPath && (
                <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
                    <div 
                        className="fixed inset-0 bg-gray-900/60 backdrop-blur-md animate-in fade-in"
                        onClick={() => setViewingProofPath(null)}
                    />
                    
                    <div className="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-[2rem] relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-700">
                        <div className="p-8 border-b border-gray-50 dark:border-gray-700 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                    Verifikasi keaslian transfer bank
                                </p>
                            </div>
                            <button 
                                onClick={() => setViewingProofPath(null)} 
                                className="p-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-400 rounded-xl transition-colors"
                            >
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        <div className="p-8 bg-gray-50 dark:bg-gray-900 max-h-[70vh] overflow-y-auto flex items-center justify-center">
                            <img 
                                src={`/storage/${viewingProofPath}`} 
                                alt="Bukti Transfer" 
                                className="max-w-full h-auto rounded-2xl shadow-md border border-gray-200 dark:border-gray-700" 
                            />
                        </div>
                    </div>
                </div>
